...
-- -----------------------------------------------------------------------------------------------
essmonitor_proto = Proto("ess_monitor","BM")
...
function essmonitor_proto.dissector(buffer, pinfo, tree)
  ...
  do
    ...
    npos     = buffer(offset + dataheadersize + 12, 4):le_uint()
	  --ypos     = buffer(offset + dataheadersize + 13, 3):le_uint()


    -- make a readout summary
    dtree = tree:add(buffer(offset, dataheadersize + datasize),
            string.format("%3d Fiber/Ring/FEN %u/%d/%d, Type: %s, " ..
						              "Channel %d, ADC %2d Pos %4d",
            readouts, fiberid, ringid, fenid, type2str(type), channel, adc, npos))
    ...
    dtree:add(buffer(offset + dataheadersize +  9, 1), string.format("Channel   %d",   channel))
	  ...
	  dtree:add(buffer(offset + dataheadersize + 12, 1), string.format("NPos      0x%04x",   npos))
