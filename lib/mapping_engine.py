...
# =============================================================================
# NMXMapper — NMX per-hybrid VMM normal mode
# =============================================================================
...
class NMXMapper(DetectorMapper):
    ...
    def map(readouts, config: dict) -> hitsVMMnormal:
        ...
        topo = {
            ...
            'ring': np.array(rings, dtype='int64'),
            'fen': np.array(fens, dtype='int64'),
            'hybrid': np.array(hybrids, dtype='int64'),
            'plane': np.array(planes, dtype='int64'),
            'offset': np.array(offsets, dtype='int64'),
            'reversedChannels': np.array(flips, dtype=bool)
        }

        n = readouts.fill_count
        src = readouts.matrix[:n]

        # Stage 1: Vectorized match on (ring, fen, hybrid)
        assigned_ids, topo_idx, valid_mask = DetectorMapper._assign_ids_vectorized(
            src, topo, src_third_field='hybrid'
        )

        plane = np.where(valid_mask, topo['plane'][topo_idx], np.int64(-1))

        # Stage 2: Global strip coordinate formula
        local = src['channel'] + np.int64(64) * src['asic']
        offset = np.where(valid_mask, topo['offset'][topo_idx], np.int64(0))
        reversed_ = np.where(valid_mask, topo['reversedChannels'][topo_idx], False)

        global_index = np.where(
            valid_mask,
            offset + np.where(reversed_, np.int64(127) - local, local),
            np.int64(-1)
        )
        ...
        _report_unmapped_units(assigned_ids, np.unique(topo['ID']), 'quadrant')
        return h
# =============================================================================
