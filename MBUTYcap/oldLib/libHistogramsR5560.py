...
class allAxis():    
    ...
    def createSpecificAxis(self,parameters,cassOffset=0):
        ...
        self.axMult = createAx(0, 9, 10)
        
        # offset = cassOffset*parameters.config.DETparameters.numOfWires
        # start  = offset
        start  = 0 
        stop   = 1 
        steps  = parameters.config.DETparameters.positionBins 
        self.axWires  = createAx(start, stop, steps)
        
        
        cf = parameters.config.DETparameters.cassInConfig
        
        mincf = np.min(cf)
        maxcf = np.max(cf)
        numCass = maxcf-mincf+1
        ...
        # self.axStrips.axis = np.array(parameters.config.DETparameters.cassInConfig)
        
        # offset_mm = cassOffset*parameters.config.DETparameters.numOfWires*parameters.config.DETparameters.wirePitch*sine
        # start  = offset_mm
        start  = 0
        stop   = parameters.config.DETparameters.tubeLength 
        steps  = parameters.config.DETparameters.positionBins
        ...
        self.axStrips_mm.axis = self.axStrips.axis*parameters.config.DETparameters.tubeSpacing
...
if __name__ == '__main__' :
    ...
    confFile  = '/Users/francescopiscitelli/Documents/PYTHON/MBUTYcap/config/'+"MIRACLES24.json"
